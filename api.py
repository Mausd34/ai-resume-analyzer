...
@app.get('/health')
def health():return {'status':'ok','service':'ai-resume-analyzer','version':'2.0.0'}
@app.get('/skills')
...
async def analyze(resume:UploadFile=File(...),job_description:str=Form(...)):
    ...
    text=pdf_text(await resume.read());r=set(extract(text));j=set(extract(job_description));matched=sorted(r&j);missing=sorted(j-r);score=round(100*len(matched)/max(1,len(j)),1)
